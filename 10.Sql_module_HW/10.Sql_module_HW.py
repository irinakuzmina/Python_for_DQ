...
import NewsToDB  # import module to save records to DB
...
class AllNews:
    ...
    def set_data(self):
        ...
        if self.type1 == 1:
            ...
            # insert input record into NEWS db table
            db.insert_news(self.text, self.city, str(self.news_date))
        ...
        if self.type1 == 2:
            ...
            # insert input record into AD db table
            db.insert_ad(self.text, self.exp_date, str(self.days_left))
        ...
        if self.type1 == 3:
            ...
            self.news_date = datetime.now().strftime("%d/%m/%Y %H:%M")
            # insert input record into EVENT db table
            db.insert_event(self.text, self.city, str(self.news_date))
            return f"\nEvent announcement-----------\n{self.text}\n{self.city}, {self.news_date}" \
                   f"\n---------------------------\n\n"
        if self.type1 == 4:
