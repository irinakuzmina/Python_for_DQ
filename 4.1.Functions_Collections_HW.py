...
def dict_list_creation():
    ...
    return rand_list  # to print to console created list of dictionaries


print(dict_list_creation())
...
def common_dict_creation():
    ...
    random_list = dict_list_creation()
    h_list = random_list

    for i in random_list:  # loop to check each dictionary from rand_list
        h_list = [
                ...
                for key, value in i.items():  # to compare keys from current dictionary with others
                    for key1, value1 in k.items():
                        if key == key1:  # if statement
                            if value >= value1:
                                # k.pop(key1)
                                key = key+'_'+str(random_list.index(i)+1)  # to rename key if 1st value is bigger
                                common_dict.update({key: value})  # to update common_dict with key+value pair
                            else:
                                # i.pop(key)
                                key1 = key1+'_'+str(random_list.index(k)+1) # to rename key if 2nd value is bigger
